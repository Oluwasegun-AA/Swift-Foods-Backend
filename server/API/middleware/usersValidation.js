...
function userIdValidation(req, res, next) {
  ...
  if (id === undefined || id === null || id === '') {
    error.userId_Error = 'Bad Request, User ID is Required';
  } else if (!(id.match(numbers))) { error.status = 'unsuccessful'; error.userId_Error = 'Bad Request, User ID is not Valid'; }
  if ((Object.keys(error).length) > 0) { return res.status(400).send({ success: false, status: 'unsuccessful', Error_Log: error }); }
  next();
}

function userPostIdValidation(req, res, next) {
  return res.status(400).send({ success: false, status: 'unsuccessful', Error: 'Bad Request, User ID is not Required' });
}
